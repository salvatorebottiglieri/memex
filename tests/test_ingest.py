...
def test_ingest_returns_json_with_node_id(store):
...
def test_ingest_writes_l0_markdown_file(store):
...
def test_l0_file_contains_fetched_content(store):
    ingest(store, "https://example.com/article")
    vault = store["vault"]
...
def test_ingest_inserts_node_row(store):
    ingest(store, "https://example.com/article")
...
def test_ingest_inserts_source_row_with_canonical_key(store):
...
def test_ingest_source_row_records_fetched_at(store):
    ingest(store, "https://example.com/article")
    con = sqlite3.connect(store["db"])
...
def test_ingesting_same_url_twice_yields_one_node(store):
    ingest(store, "https://example.com/article")
...
def test_ingesting_same_url_twice_second_returns_already_exists(store):
    ingest(store, "https://example.com/article")
    result2 = ingest(store, "https://example.com/article")
...
def test_tracking_param_variant_deduped_to_same_node(store):
...
def test_already_exists_response_includes_failed_false_for_successful_prior_ingest(store):
    ingest(store, "https://example.com/article")
    result2 = ingest(store, "https://example.com/article")
    data = json.loads(result2.stdout)
    assert data["status"] == "already_exists"
...
def test_already_exists_response_includes_failed_true_for_prior_failed_ingest(store):
    ingest(store, "https://fail.example.com/article")
    result2 = ingest(store, "https://fail.example.com/article")
    data = json.loads(result2.stdout)
    assert data["status"] == "already_exists"
    assert data["failed"] is True


def test_fetch_failure_does_not_crash(store):
    result = ingest(store, "https://fail.example.com/article")
    assert result.returncode == 0, result.stderr


def test_fetch_failure_returns_json_with_failed_status(store):
    result = ingest(store, "https://fail.example.com/article")
    data = json.loads(result.stdout)
    assert data["status"] == "fetch_failed"


def test_fetch_failure_records_source_row_with_failed_flag(store):
    ingest(store, "https://fail.example.com/article")
    con = sqlite3.connect(store["db"])
    row = con.execute(
